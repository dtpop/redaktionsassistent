 ...
 * 
 * Spezialversion. Prüft, ob im Wert rex_article etwas drinsteht und macht das Feld dann readonly
 * 
 */

class rex_yform_value_date_ra extends rex_yform_value_abstract
{
    const
    ...
    public function enterObject()
    {
        ...
        // Nur speichern, wenn es noch keinen Redaxo Artikel gibt
        if ($this->saveInDb()) {
            if ($params['rex_article']) {
                $article = rex_article::get($params['rex_article']);
                if ($article instanceof rex_article) {
                    $this->params['value_pool']['sql'][$this->getName()] = date('Y-m-d',$article->getValue('art_online_from'));
                }
            } else {
